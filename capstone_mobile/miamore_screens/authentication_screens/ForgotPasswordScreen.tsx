...
const ForgotPasswordScreen = () => {
  ...
  const handleSendOtp = async () => {
    try {
      const res = await fetch('http://10.0.2.2:5000/api/forgot-password', {
        method: 'POST',
...
const styles = StyleSheet.create({
  ...
  backArrow: { fontSize: 32, color: '#5A5A5A' },
  title: { fontSize: 22, fontWeight: 'bold', marginBottom: 20 },
  subtitle: { fontSize: 14, color: '#666', marginBottom: 30 },
  input: { borderWidth: 1, borderColor: '#ccc', borderRadius: 8, padding: 12, marginBottom: 30 },
  button: { backgroundColor: '#92e3a9', padding: 15, borderRadius: 8, alignItems: 'center', elevation: 3 },
